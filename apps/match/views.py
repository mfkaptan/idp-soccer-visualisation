...
class MatchDetail(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for f in home_framesets:
            ...
            home[f.player.shirt_number] = list(f.frame_set.filter(m__lt=10).order_by('n').values('x', 'y', 'n'))
        ...
        for f in away_framesets:
            ...
            away[f.player.shirt_number] = list(f.frame_set.filter(m__lt=10).order_by('n').values('x', 'y', 'n'))
        ...
        raw_data = serializers.serialize('json', ball_frameset.frame_set.filter(m__lt=10).order_by('n'),
                                         fields=('x', 'y', 'z', 'm', 'n', 't'))
